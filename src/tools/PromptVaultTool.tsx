    ...
    <div className="space-y-4 h-full flex flex-col">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-xl font-bold italic">提示词宝库</h2>
        </div>
      ...
      <div className="glass-card p-4 rounded-xl border border-border space-y-3">
        ...
        <div className="space-y-2">
          <div className="text-xs text-muted-foreground">Prompt（必填）</div>
          <textarea
            value={prompt}
            onChange={(e) => setPrompt(clampPrompt(e.target.value))}
            placeholder="粘贴你的 Prompt..."
            className={cn(
              'w-full min-h-[140px] resize-y rounded-lg border border-border bg-muted/10',
          ...
          <div className="flex items-center justify-between text-[11px] text-muted-foreground">
            ...
            <Button size="sm" onClick={handleSubmit} disabled={submitting || !title.trim() || !prompt.trim()} className="space-x-1">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-[11px] text-muted-foreground mt-2">
                      贡献者：{it.created_by_email || it.created_by} · {formatLocal(it.created_at)}
                    </div>
                  </div>
